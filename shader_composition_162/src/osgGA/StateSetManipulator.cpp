...
void StateSetManipulator::setLightingEnabled(bool newlighting)
{
    ...
    if( _lighting ) 
	{
		...
		_stateset->setShaderMode(ShaderModeIndexPair(ShaderModeLighting::type, 0), osg::StateAttribute::ON);
	}
    else 
	{
		_stateset->setMode(GL_LIGHTING,osg::StateAttribute::OVERRIDE|osg::StateAttribute::OFF);
		_stateset->setShaderMode(ShaderModeIndexPair(ShaderModeLighting::type, 0), osg::StateAttribute::OVERRIDE|osg::StateAttribute::OFF);
	}
}
...
//    cout << tex->numTextureUnits() << endl;
    ...
    for( unsigned int ii=0; ii<_maxNumOfTextureUnits; ii++ )
    {
		...
		_stateset->setShaderMode(ShaderModeIndexPair(ShaderModeTexture2D::type, ii), mode );
    }
}
