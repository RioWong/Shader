    ...
    _nestRenderBins(rhs._nestRenderBins)
{
	...
	_shaderModeList = rhs._shaderModeList;
...
int StateSet::compare(const StateSet& rhs,bool compareAttributeContents) const
{
	...
	// check shader modes
    ShaderModeList::const_iterator lhs_shadermode_itr = _shaderModeList.begin();
    ShaderModeList::const_iterator rhs_shadermode_itr = rhs._shaderModeList.begin();
    while (lhs_shadermode_itr!=_shaderModeList.end() && rhs_shadermode_itr!=rhs._shaderModeList.end())
    {
        if      (lhs_shadermode_itr->first<rhs_shadermode_itr->first) return -1;
        else if (rhs_shadermode_itr->first<lhs_shadermode_itr->first) return 1;
        if      (lhs_shadermode_itr->second<rhs_shadermode_itr->second) return -1;
        else if (rhs_shadermode_itr->second<lhs_shadermode_itr->second) return 1;
        ++lhs_shadermode_itr;
        ++rhs_shadermode_itr;
    }
    if (lhs_shadermode_itr==_shaderModeList.end())
    {
        if (rhs_shadermode_itr!=rhs._shaderModeList.end()) return -1;
    }
    else if (rhs_shadermode_itr == rhs._shaderModeList.end()) return 1;
...
int StateSet::compareShaderModes(const ShaderModeList& lhs,const ShaderModeList& rhs)
{
    ShaderModeList::const_iterator lhs_mode_itr = lhs.begin();
    ShaderModeList::const_iterator rhs_mode_itr = rhs.begin();
    while (lhs_mode_itr!=lhs.end() && rhs_mode_itr!=rhs.end())
    {
        if      (lhs_mode_itr->first<rhs_mode_itr->first) return -1;
        else if (rhs_mode_itr->first<lhs_mode_itr->first) return 1;
...
{    
    ...
    #endif
	setShaderMode(ShaderModeIndexPair(osg::ShaderModeMaterial::type, 0), StateAttribute::ON);
}
...
void StateSet::merge(const StateSet& rhs)
{
    ...
    // merge the shadermodes of rhs into this, 
    // this overrides rhs if OVERRIDE defined in this.
    for(ShaderModeList::const_iterator rhs_mitr = rhs._shaderModeList.begin();
        rhs_mitr != rhs._shaderModeList.end();
        ++rhs_mitr)
    {
        ShaderModeList::iterator lhs_mitr = _shaderModeList.find(rhs_mitr->first);
        if (lhs_mitr!=_shaderModeList.end())
        {
            // take the rhs mode unless the lhs is override and the rhs is not protected 
            if (!(lhs_mitr->second & StateAttribute::OVERRIDE ) ||
                 (rhs_mitr->second & StateAttribute::PROTECTED)) 
        ...
        {
            // entry doesn't exist so insert it.
            _shaderModeList.insert(*rhs_mitr);
        }
    }
...
void StateSet::setShaderMode(ShaderModeList& shaderModeList, ShaderModeIndexPair shaderMode, StateAttribute::GLModeValue value)
{
    if ((value&StateAttribute::INHERIT)) setShaderModeToInherit(shaderModeList, shaderMode);
    else shaderModeList[shaderMode] = value;
}

void StateSet::setShaderModeToInherit(ShaderModeList& shaderModeList, ShaderModeIndexPair shaderMode)
{
    ShaderModeList::iterator itr = shaderModeList.find(shaderMode);
    if (itr!=shaderModeList.end())
    {
        shaderModeList.erase(itr);
    }
}

StateAttribute::GLModeValue StateSet::getShaderMode(const ShaderModeList& shaderModeList, ShaderModeIndexPair shaderMode) const
{
    ShaderModeList::const_iterator itr = shaderModeList.find(shaderMode);
    if (itr!=shaderModeList.end())
    {
        return itr->second;
    }
...
class SetAssociateModesHelper : public StateAttribute::ModeUsage
{
    public:
        ...
        virtual void usesShaderMode(ShaderMode::Type shaderMode)
        {
			_stateset->setShaderMode(ShaderModeIndexPair(shaderMode, _unit),_value);
		}

        virtual void usesShaderMode(ShaderModeIndexPair shaderMode)
        {
            _stateset->setShaderMode(shaderMode,_value);
        }
...
class RemoveAssociateModesHelper : public StateAttribute::ModeUsage
{
    public:
		...
		virtual void usesShaderMode(ShaderMode::Type shaderMode)
		{
			_stateset->removeShaderMode(ShaderModeIndexPair(shaderMode, _unit));
		}

		virtual void usesShaderMode(ShaderModeIndexPair shaderMode)
        {
            _stateset->removeShaderMode(shaderMode);
        }
