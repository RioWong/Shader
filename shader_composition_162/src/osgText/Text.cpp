...
//	state.applyShaderMode(ShaderModeIndexPair(osg::ShaderMode::TEXTURE2D,0), true);

	// Prevent shader program usage, 
	// Text implementation does not use state.apply and therefore no proper shader program will be created
	// Need a default shader program to handle things like text and other locations that directly 
	// manipulate the state
	state.applyAttribute(new Program);
#else
...
//			state.applyShaderMode(ShaderModeIndexPair(osg::ShaderMode::TEXTURE2D, 0), false);
